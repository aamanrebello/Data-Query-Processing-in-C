...
SubDB* hash_join(const SubDB* left, const SubDB* right){
    ...
    SubDB* output = malloc(SIZE_OF_SUBDB);
    ...
    if(no_of_partitions == 0)
    {
      ...
      hash_table* htemp = malloc(SIZE_OF_HASHTABLE);
...
int main()
{
  //Test 1 - single isomorphic triangle
  void* m1 = malloc(sizeof(MainDB));
  allocate_maindb(m1, 32, 3);
  insert_maindb(m1, 0, 1, 0);
  insert_maindb(m1, 1, 2, 0);
  insert_maindb(m1, 2, 0, 0);

  SubDB* s11 = create_subdb(m1, 0, 1, 0);
  SubDB* s12 = create_subdb(m1, 0, 1, 1);

  sort_table_subdb(s11, false);
  sort_table_subdb(s12, true);
  SubDB* sr11 = sort_merge(s11, s12);

  //SubDB* sr11 = hash_join(s11, s12);

  deallocate_subdb(s11); free(s11);
  deallocate_subdb(s12); free(s12);

  SubDB* s13 = create_subdb(m1, 0, 1, 0);

  sort_table_subdb(sr11, false);
  sort_table_subdb(s13, true);
  SubDB* result1 = sort_merge(sr11, s13);

  //SubDB* result1 = hash_join(sr11, s13);

  printf("\nRESULT: %ld", check_equal_lr_subdb(result1));

  deallocate_subdb(s13); free(s13);
  deallocate_subdb(result1); free(result1);

  deallocate_maindb(m1);
  free(m1);


  //Test 2 - "Single, non-self-isomorphic triangle"
  void* m2 = malloc(sizeof(MainDB));
  allocate_maindb(m2, 32, 3);

  insert_maindb(m2, 0, 1, 0);
  insert_maindb(m2, 1, 2, 1);
  insert_maindb(m2, 2, 0, 2);

  SubDB* s21 = create_subdb(m2, 0, 1, 0);
  SubDB* s22 = create_subdb(m2, 0, 1, 0);

  sort_table_subdb(s21, false);
  sort_table_subdb(s22, true);
  SubDB* sr21 = sort_merge(s21, s22);

  //SubDB* sr21 = hash_join(s21, s22);

  deallocate_subdb(s21); free(s21);
  deallocate_subdb(s22); free(s22);

  SubDB* s23 = create_subdb(m2, 0, 1, 0);

  sort_table_subdb(sr21, false);
  sort_table_subdb(s23, true);
  SubDB* result2 = sort_merge(sr21, s23);

  //SubDB* result2 = hash_join(sr21, s23);

  printf("\nRESULT: %ld", check_equal_lr_subdb(result2));

  deallocate_subdb(s23); free(s23);
  deallocate_subdb(result2); free(result2);

  deallocate_maindb(m2);
  free(m2);


  // Test 3 - "Multiple, non-self-isomorphic triangle"
  void* m3 = malloc(sizeof(MainDB));
  allocate_maindb(m3, 32, 3);

  insert_maindb(m3, 0, 1, 0);
  insert_maindb(m3, 0, 2, 0);
  insert_maindb(m3, 0, 3, 0);
  insert_maindb(m3, 1, 3, 1);
  insert_maindb(m3, 1, 4, 1);
  insert_maindb(m3, 1, 2, 1);
  insert_maindb(m3, 3, 4, 1);
  insert_maindb(m3, 3, 2, 1);
  insert_maindb(m3, 2, 0, 2);
  insert_maindb(m3, 4, 0, 2);
  insert_maindb(m3, 3, 0, 2);

  SubDB* s31 = create_subdb(m3, 0, 1, 0);
  SubDB* s32 = create_subdb(m3, 0, 1, 1);

  sort_table_subdb(s31, false);
  sort_table_subdb(s32, true);
  SubDB* sr31 = sort_merge(s31, s32);

//  SubDB* sr31 = hash_join(s31, s32);

  deallocate_subdb(s31); free(s31);
  deallocate_subdb(s32); free(s32);

  SubDB* s33 = create_subdb(m3, 0, 1, 2);

  sort_table_subdb(sr31, false);
  sort_table_subdb(s33, true);
  SubDB* result3 = sort_merge(sr31, s33);

  //SubDB* result3 = hash_join(sr31, s33);

  printf("\nRESULT: %ld", check_equal_lr_subdb(result3));

  deallocate_subdb(s33); free(s33);
  deallocate_subdb(result3); free(result3);

  deallocate_maindb(m3);
  free(m3);

  //Test 4 - "Multiple, deleted edge"
  void* m4 = malloc(sizeof(MainDB));
  allocate_maindb(m4, 32, 3);

  insert_maindb(m4, 0, 1, 0);
  insert_maindb(m4, 0, 2, 0);
  insert_maindb(m4, 0, 3, 0);
  insert_maindb(m4, 1, 3, 1);
  insert_maindb(m4, 1, 4, 1);
  insert_maindb(m4, 1, 2, 1);
  insert_maindb(m4, 3, 4, 1);
  insert_maindb(m4, 3, 2, 1);
  insert_maindb(m4, 2, 0, 2);
  delete_maindb(m4, 1, 3, 1);
  delete_maindb(m4, 3, 4, 1);
  insert_maindb(m4, 4, 0, 2);
  insert_maindb(m4, 3, 0, 2);
  insert_maindb(m4, 3, 4, 1);

  SubDB* s41 = create_subdb(m4, 0, 1, 0);
  SubDB* s42 = create_subdb(m4, 0, 1, 1);

  sort_table_subdb(s41, false);
  sort_table_subdb(s42, true);
  SubDB* sr41 = sort_merge(s41, s42);

  //SubDB* sr41 = hash_join(s41, s42);

  deallocate_subdb(s41); free(s41);
  deallocate_subdb(s42); free(s42);

  SubDB* s43 = create_subdb(m4, 0, 1, 2);

  sort_table_subdb(sr41, false);
  sort_table_subdb(s43, true);
  SubDB* result4 = sort_merge(sr41, s43);

  //SubDB* result4 = hash_join(sr41, s43);

  printf("\nRESULT: %ld", check_equal_lr_subdb(result4));

  deallocate_subdb(s43); free(s43);
  deallocate_subdb(result4); free(result4);

  deallocate_maindb(m4);
  free(m4);

  //Test 5 - "Multiple, grouped edges"
  void* m5 = malloc(sizeof(MainDB));
  allocate_maindb(m5, 32, 3);

  insert_maindb(m5, 0, 1, 0);
  insert_maindb(m5, 0, 1, 1);
  insert_maindb(m5, 0, 2, 0);
  insert_maindb(m5, 0, 2, 1);
  insert_maindb(m5, 0, 3, 0);
  insert_maindb(m5, 0, 3, 1);
  insert_maindb(m5, 0, 3, 2);
  insert_maindb(m5, 1, 3, 1);
  insert_maindb(m5, 1, 3, 2);
  insert_maindb(m5, 1, 4, 1);
  insert_maindb(m5, 1, 4, 2);
  insert_maindb(m5, 1, 2, 1);
  insert_maindb(m5, 1, 2, 2);
  insert_maindb(m5, 3, 4, 1);
  insert_maindb(m5, 3, 4, 2);
  insert_maindb(m5, 3, 2, 1);
  insert_maindb(m5, 3, 2, 2);
  insert_maindb(m5, 2, 0, 2);
  insert_maindb(m5, 2, 0, 3);
  delete_maindb(m5, 1, 3, 1);
  delete_maindb(m5, 3, 4, 1);
  insert_maindb(m5, 4, 0, 2);
  insert_maindb(m5, 4, 0, 3);
  insert_maindb(m5, 3, 0, 2);
  insert_maindb(m5, 3, 0, 3);
  insert_maindb(m5, 3, 4, 1);
  insert_maindb(m5, 3, 4, 3);

  SubDB* s51 = create_subdb(m5, 0, 1, 0);
  SubDB* s52 = create_subdb(m5, 0, 1, 1);

  sort_table_subdb(s51, false);
  sort_table_subdb(s52, true);
  SubDB* sr51 = sort_merge(s51, s52);

  //SubDB* sr51 = hash_join(s51, s52);

  deallocate_subdb(s51); free(s51);
  deallocate_subdb(s52); free(s52);

  SubDB* s53 = create_subdb(m5, 0, 1, 2);

  sort_table_subdb(sr51, false);
  sort_table_subdb(s53, true);
  SubDB* result5 = sort_merge(sr51, s53);

  //SubDB* result5 = hash_join(sr51, s53);

  printf("\nRESULT: %ld", check_equal_lr_subdb(result5));

  deallocate_subdb(s53); free(s53);
  deallocate_subdb(result5); free(result5);

  deallocate_maindb(m5);
  free(m5);
}
